...
if st.button("🚨 Scan for History Trend Opportunities"):
    ...
    for keyword in keywords:
        ...
        # 2️⃣ Fetch full video details (statistics + duration)
        ...
        for vid_item in video_data.get("items", []):
            ...
            video_map[vid_id] = {
                "views": int(vid_item["statistics"].get("viewCount", 0)),
                "duration": iso_duration_to_seconds(duration_iso)
            }

        # 3️⃣ Fetch channel statistics
        ...
        # 4️⃣ Process each search result
        for v in videos:
            vid = v["id"]["videoId"]
            ...
            # 5️⃣ Correct timezone-aware datetime
